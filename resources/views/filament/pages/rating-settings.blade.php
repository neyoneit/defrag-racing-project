    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
            ...
            <button
                ...
                wire:target="runRecalc"
                style="background: #16a34a; color: #fff; padding: 12px 32px; font-size: 14px; font-weight: 800; border-radius: 8px; border: none; cursor: pointer; text-transform: uppercase; letter-spacing: 1px; width: 100%; display: flex; align-items: center; justify-content: center; gap: 8px;"
                onmouseover="this.style.background='#22c55e'"
                onmouseout="this.style.background='#16a34a'"
            >
                <span wire:loading wire:target="runRecalc">
                ...
                <span wire:loading.remove wire:target="runRecalc">RUN RECALCULATION</span>
                <span wire:loading wire:target="runRecalc">RUNNING...</span>
            </button>
            ...
            @if($recalcOutput)
                <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-4 max-h-[500px] overflow-y-auto">
                    <h3 class="text-xs font-bold text-green-400 uppercase tracking-wider mb-2">Output</h3>
                    <pre class="text-[11px] font-mono text-gray-400 whitespace-pre-wrap leading-relaxed">{{ $recalcOutput }}</pre>
                </div>
            @endif
        </div>
    </div>
