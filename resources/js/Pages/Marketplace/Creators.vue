...
const selectedSpecialtyLabel = () => specialties.value.find(s => s.value === specialtyFilter.value)?.label || t('All Specialties');
const selectedSortLabel = () => sorts.value.find(s => s.value === sortBy.value)?.label || t('Newest');
...
<template>
    <div class="pb-4">
        <Head :title="$t('Creator Directory - Marketplace')" />
            ...
            <div class="max-w-8xl mx-auto px-4 md:px-6 lg:px-8 pointer-events-auto">
                <div class="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <h1 class="text-2xl md:text-3xl font-black text-gray-300/90 mb-2">{{ $t('Creator Directory') }}</h1>
                        <p class="text-gray-400">{{ $t('Browse community creators available for commissions') }}</p>
                    </div>
                </div>
            </div>
        </div>
