<script setup>
    import { Head, Link, router } from '@inertiajs/vue3';
    import { ref, onMounted, computed } from 'vue';
    import TournamentCard from '@/Components/TournamentCard.vue';
    import Tabs from '@/Components/Tabs.vue';
    import { t } from '@/utils/i18n';

    const props = defineProps({
        tournaments: Object,
        activeTournaments: Object,
        upcomingTournaments: Object,
        pastTournaments: Object,
    });

    const tournamentsLoaded = ref(false);

    onMounted(() => {
        if (!props.tournaments) {
            const start = Date.now();
            router.reload({
                only: ['tournaments', 'activeTournaments', 'upcomingTournaments', 'pastTournaments'],
                onFinish: () => {
                    const remaining = 400 - (Date.now() - start);
                    if (remaining > 0) {
                        setTimeout(() => { tournamentsLoaded.value = true; }, remaining);
                    } else {
                        tournamentsLoaded.value = true;
                    }
                }
            });
        } else {
            tournamentsLoaded.value = true;
        }
    });

    const filteredTournaments = ref([]);

    const searchInput = ref('');

    const tabs = computed(() => [
        {
            name: 'Active Tournaments',
            label: t('Active Tournaments'),
            link: false,
        },
        {
            name: 'Upcoming Tournaments',
            label: t('Upcoming Tournaments'),
            link: false,
        },
        {
            name: 'Historical Tournaments',
            label: t('Historical Tournaments'),
            link: false,
        },
        {
            name: 'Search Tournaments',
            label: t('Search Tournaments'),
            link: false,
        }
    ]);

    const activeTab = ref('Active Tournaments');

    const toggleTab = (tab) => {
        activeTab.value = tab;
    };

    const onSearchTournament = () => {
        if (searchInput.value.length > 0) {
            filteredTournaments.value = (props.tournaments || []).filter((tournament) => {
                return tournament.name.toLowerCase().includes(searchInput.value.toLowerCase());
            });
        } else {
            filteredTournaments.value = [];
        }
    };
</script>

<template>
    <div>
        <Head :title="$t('Tournaments')" />

        <!-- Top Shadow Gradient -->
        <div class="relative bg-gradient-to-b from-black/25 via-black/10 to-transparent pt-6 pb-96 pointer-events-none">
            <div class="max-w-8xl mx-auto px-4 md:px-6 lg:px-8 pointer-events-auto">
                <!-- Header -->
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
                <div>
                    <h1 class="text-2xl md:text-3xl font-black text-gray-300/90 mb-2">{{ $t('Tournaments') }}</h1>
                    <p class="text-gray-400">{{ $t('Compete in defrag tournaments and climb the rankings') }}</p>
                </div>

                <!-- Create Tournament Button -->
                <Link
                    v-if="$page.props.auth.user && !$page.props.isVerified"
                    href="/email/verify"
                    class="flex items-center gap-2 px-4 py-3 bg-red-600/80 hover:bg-red-500 text-white font-bold rounded-lg transition-all text-sm"
                >
                    {{ $t('Verify Email to Create') }}
                </Link>

                <Link
                    v-else-if="$page.props.auth.user && $page.props.auth.user.mdd_id"
                    :href="route('tournaments.create')"
                    class="flex items-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition-all transform hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-blue-500/20"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                    {{ $t('Create Tournament') }}
                </Link>

                <Link
                    v-else-if="!$page.props.auth.user"
                    :href="route('login')"
                    class="flex items-center gap-2 px-4 py-3 bg-gray-700/50 border border-white/10 text-gray-400 font-bold rounded-lg transition-all hover:bg-gray-600/50 hover:text-gray-300"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
                    </svg>
                    {{ $t('Log in to create tournaments') }}
                </Link>

                <Link
                    v-else
                    :href="route('settings.show')"
                    class="flex items-center gap-2 px-4 py-3 bg-gray-700/50 border border-white/10 text-gray-400 font-bold rounded-lg transition-all hover:bg-gray-600/50 hover:text-gray-300"
                    :title="$t('Link your account to create tournaments')"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m9.86-3.062a4.5 4.5 0 0 0-1.242-7.244l4.5-4.5a4.5 4.5 0 0 1 6.364 6.364l-1.757 1.757" />
                    </svg>
                    {{ $t('Link Account to Create') }}
                </Link>
            </div>
            </div>
        </div>

        <!-- Modern Tournaments Container -->
        <div class="max-w-8xl mx-auto px-4 md:px-6 lg:px-8" style="margin-top: -22rem;">
            <!-- Verify Email Warning -->
            <div v-if="$page.props.auth?.user && !$page.props.isVerified" class="bg-red-500/10 backdrop-blur-sm border border-red-500/30 rounded-xl p-4 mb-8">
                <div class="flex items-start gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6 text-red-400 shrink-0 mt-0.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
                    </svg>
                    <div class="flex-1">
                        <p class="text-red-400 font-bold mb-1">{{ $t('Email Verification Required') }}</p>
                        <p class="text-red-300 text-sm [&_a]:text-red-200 [&_a:hover]:text-white [&_a]:underline [&_a]:font-medium"
                            v-html="$t('<a href=/email/verify>Verify your email</a> to create and participate in tournaments.')"></p>
                    </div>
                </div>
            </div>
            <!-- Link Account Warning (only for verified users without linked account) -->
            <div v-else-if="$page.props.auth?.user && !$page.props.auth?.user?.mdd_id" class="bg-yellow-500/10 backdrop-blur-sm border border-yellow-500/30 rounded-xl p-4 mb-8">
                <div class="flex items-start gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6 text-yellow-400 shrink-0 mt-0.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m9.86-3.062a4.5 4.5 0 0 0-1.242-7.244l4.5-4.5a4.5 4.5 0 0 1 6.364 6.364l-1.757 1.757" />
                    </svg>
                    <div class="flex-1">
                        <p class="text-yellow-400 font-bold mb-1">{{ $t('Account Linking Required') }}</p>
                        <p class="text-yellow-300 text-sm [&_a]:text-yellow-200 [&_a:hover]:text-white [&_a]:underline [&_a]:font-medium"
                            v-html="$t('<a href=:url>Link your account</a> to Q3DF.org to create tournaments.', { url: route('settings.show') })"></p>
                    </div>
                </div>
            </div>

            <!-- Tabs and Content -->
            <div class="bg-black/40 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/10 overflow-hidden">
                <!-- Tabs Header -->
                <Tabs :tabs="tabs" :onClick="toggleTab" :activeTab="activeTab" />

                <!-- Tab Content -->
                <div class="p-6">
                    <!-- Loading skeleton -->
                    <div v-if="!tournamentsLoaded" class="space-y-4">
                        <div v-for="i in 3" :key="i" class="bg-black/40 backdrop-blur-sm rounded-xl p-6 border border-white/10 animate-pulse">
                            <div class="h-6 bg-white/10 rounded w-1/3 mb-3"></div>
                            <div class="h-4 bg-white/5 rounded w-2/3 mb-2"></div>
                            <div class="h-4 bg-white/5 rounded w-1/2"></div>
                        </div>
                    </div>

                    <!-- Active Tournaments -->
                    <div v-show="tournamentsLoaded && activeTab === 'Active Tournaments'" id="active-tab" class="space-y-4">
                        <div v-for="(tournament, id) in (activeTournaments || [])" :key="id">
                            <TournamentCard :tournament="tournament" />
                        </div>
                        <div v-if="activeTournaments && activeTournaments.length === 0" class="text-center py-12">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-gray-600 mx-auto mb-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 0 1 3 3h-15a3 3 0 0 1 3-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 0 1-.982-3.172M9.497 14.25a7.454 7.454 0 0 0 .981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 0 0 7.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M7.73 9.728a6.726 6.726 0 0 0 2.748 1.35m8.272-6.842V4.5c0 2.108-.966 3.99-2.48 5.228m2.48-5.492a46.32 46.32 0 0 1 2.916.52 6.003 6.003 0 0 1-5.395 4.972m0 0a6.726 6.726 0 0 1-2.749 1.35m0 0a6.772 6.772 0 0 1-3.044 0" />
                            </svg>
                            <p class="text-gray-400 font-medium">{{ $t('No active tournaments available') }}</p>
                        </div>
                    </div>

                    <!-- Upcoming Tournaments -->
                    <div v-show="tournamentsLoaded && activeTab === 'Upcoming Tournaments'" id="upcoming-tab" class="space-y-4">
                        <div v-for="(tournament, id) in (upcomingTournaments || [])" :key="id">
                            <TournamentCard :tournament="tournament" />
                        </div>
                        <div v-if="upcomingTournaments && upcomingTournaments.length === 0" class="text-center py-12">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-gray-600 mx-auto mb-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" />
                            </svg>
                            <p class="text-gray-400 font-medium">{{ $t('No upcoming tournaments available') }}</p>
                        </div>
                    </div>

                    <!-- Historical Tournaments -->
                    <div v-show="activeTab === 'Historical Tournaments'" id="historical-tab" class="space-y-4">
                        <div class="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 mb-6">
                            <p class="text-blue-300 text-sm text-center [&_a]:text-blue-200 [&_a:hover]:text-white [&_a]:underline [&_a]:font-medium"
                                v-html="$t('Want to submit a historical tournament? Contact <a href=:url>:name</a> on Discord.', { url: route('profile.index', 8), name: q3tohtml('^4[^7gt^4]^7neiT^7.') })"></p>
                        </div>

                        <div v-for="(tournament, id) in (pastTournaments || [])" :key="id">
                            <TournamentCard :tournament="tournament" />
                        </div>
                        <div v-if="pastTournaments && pastTournaments.length === 0" class="text-center py-12">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-gray-600 mx-auto mb-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
                            </svg>
                            <p class="text-gray-400 font-medium">{{ $t('No historical tournaments available') }}</p>
                        </div>
                    </div>

                    <!-- Search Tournaments -->
                    <div v-show="activeTab === 'Search Tournaments'" id="search-tab">
                        <div class="mb-6">
                            <input
                                @input="onSearchTournament"
                                v-model="searchInput"
                                :placeholder="$t('Search tournaments...')"
                                type="text"
                                class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-blue-500/50 focus:bg-white/10 transition-all"
                            />
                        </div>

                        <div class="space-y-4">
                            <div v-for="tournament in filteredTournaments" :key="tournament.id">
                                <TournamentCard :tournament="tournament" />
                            </div>
                        </div>

                        <div v-if="filteredTournaments.length === 0 && searchInput.length > 0" class="text-center py-12">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-gray-600 mx-auto mb-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                            </svg>
                            <p class="text-gray-400 font-medium">{{ $t('No tournaments found') }}</p>
                            <p class="text-gray-500 text-sm mt-1">{{ $t('Try a different search term') }}</p>
                        </div>

                        <div v-if="searchInput.length === 0" class="text-center py-12">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16 text-gray-600 mx-auto mb-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                            </svg>
                            <p class="text-gray-400 font-medium">{{ $t('Start typing to search') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>
