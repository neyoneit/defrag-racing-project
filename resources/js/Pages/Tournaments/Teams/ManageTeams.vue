...
<template>
    <Tournament :tournament="tournament" tab="ManageTeams">
        <div v-if="! team" class="text-center">
            <div class="text-gray-400 mb-5">{{ $t('You are not a part of any team.') }}</div>
        ...
        <div>
            <div class="text-2xl text-center text-gray-300 mt-10">{{ $t('Invitations') }}</div>

            <div class="text-center text-gray-500" v-if="invitations.length === 0">
                {{ $t('You have no team invitations yet.') }}
            </div>
            ...
            <div v-else class="mt-2">
                <div v-for="invitation in invitations" class="flex justify-between flex-wrap items-center p-3 rounded-md bg-grayop-700 text-gray-400 mb-3">
                    <div
                        class="[&>span]:mx-2 [&>span]:bg-blackop-30 [&>span]:px-3 [&>span]:py-1 [&>span]:rounded-md [&>strong]:font-normal"
                        :class="invitation.cpm ? '[&>strong]:text-green-300' : '[&>strong]:text-blue-500'"
                        v-html="$t('You are invited to join <span>:team</span> As a <strong>:physics</strong> Player', { team: q3tohtml(invitation.team.name), physics: invitation.cpm ? 'CPM' : 'VQ3' })"></div>

                    <div class="flex">
                        <div v-if="!team" @click="acceptInvitation(invitation.id)" :title="$t('Accept')" class="text-white bg-green-700 cursor-pointer hover:bg-green-600 text-center rounded-full flex items-center p-1 mr-2">
