...
// settings are recorded before the run starts, so a demo with nothing in it
// answers the question perfectly - but a page that then says "no finish time"
// reads as a failure. So the run is a separate, calm block, and its empty
// state says out loud that this is fine.
//
...
// trusting the page that finds a real one.
import { ref } from 'vue';
...
<template>
    <div>
        ...
        <!-- Same header shape as every other section: the gradient strip, the
             8xl container, the title top-aligned. -->
            ...
            <div class="max-w-8xl mx-auto px-4 md:px-6 lg:px-8">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <Link :href="route('comps.index')"
                          class="inline-flex flex-shrink-0 items-center gap-1.5 rounded-lg border border-white/15 bg-white/[0.06] px-2.5 py-1 text-sm font-bold text-gray-200 transition-colors hover:border-white/25 hover:bg-white/10">
                        {{ $t('Weekly comps') }}
                    </Link>
                </div>
            </div>
        ...
        <div class="max-w-8xl mx-auto px-4 md:px-6 lg:px-8 pb-12">
            <div class="mx-auto max-w-3xl space-y-4">
                ...
                >
                    ...
                    <div class="flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed px-6 py-10 text-center transition"
                        ...
                        <span class="text-base font-bold text-gray-200">{{ busy ? $t('Reading the demo...') : $t('Drop a demo here, or click to pick one') }}</span>
                        <span class="text-sm text-gray-400">
                            <strong class="text-gray-300">{{ $t('You do not need a finished run.') }}</strong>
                            {{ $t('Record one second and stop, that is enough.') }}
                        </span>
                        <span v-if="file && !busy" class="mt-1 break-all font-mono text-xs text-gray-500">{{ file.name }}</span>
                    </div>
                </label>
                ...
                <template v-if="result">
                    ...
                    >
                        <div class="text-lg font-black" :class="result.ok ? 'text-green-300' : 'text-amber-200'">
                    ...
                    <div class="overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-white/[0.02] backdrop-blur-sm">
                                    ...
                                    >
                                        ...
                                        <td class="px-5 py-2.5 text-right align-top">
                                            <span v-if="rule.state === 'bad'"
                                                  class="inline-flex items-center gap-1.5 rounded-md bg-red-500/20 px-2 py-0.5 font-mono text-[13px] font-black text-red-200 ring-1 ring-inset ring-red-400/30">
                                                {{ rule.found }}
                                            </span>
                        ...
                        <div v-if="result.unknown" class="border-t border-white/10 px-5 py-2.5 text-xs leading-relaxed text-gray-500">
                    ...
                    <div class="rounded-xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-white/[0.02] backdrop-blur-sm px-5 py-4">
                        ...
                        <div v-if="result.run.time_ms" class="mt-2 flex flex-wrap gap-x-8 gap-y-1 text-sm">
                            <span class="text-gray-500">{{ $t('Map') }} <span class="ml-1 font-bold text-white">{{ result.run.map || '-' }}</span></span>
                            <span class="text-gray-500">{{ $t('Physics') }} <span class="ml-1 font-bold uppercase text-white">{{ result.run.physics || '-' }}</span></span>
                            <span class="text-gray-500">{{ $t('Time') }} <span class="ml-1 font-mono font-bold tabular-nums text-white">{{ formatTime(result.run.time_ms) }}</span></span>
                        </div>
                        ...
                        <div class="mt-3 border-t border-white/5 pt-2.5 text-xs leading-relaxed text-gray-500">
