...
// Header items - migrate old 'badges' group to individual badge items
const headerItems = ref(
    (() => {
        let saved = savedLayout?.header_items;

        // Migrate old 'badges' item to individual badge items
        if (saved && saved.find(h => h.id === 'badges')) {
...
const row2Items = computed({
    get: () => headerItems.value.filter(h => h.row === 2 && availableBadges.value.includes(h.id)),
    set: (val) => {
            ...
            <div class="mb-3">
                ...
                <div class="mb-1.5">
                    ...
                    <draggable
                        ...
                        animation="150"
                        class="flex flex-wrap gap-1.5 min-h-[36px] p-2 rounded-lg border border-blue-500/20 bg-blue-950/10"
                    >
