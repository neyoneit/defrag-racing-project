...
const textFields = computed(() => [
    { key: 'text_player_name', label: t('Player Name') },
    { key: 'text_subtitle', label: t('Subtitle') },
    { key: 'text_count', label: t('Count (X / Y)') },
    { key: 'text_label', label: t('Completed Label') },
    { key: 'text_percentage', label: t('Percentage') },
    { key: 'text_remaining', label: t('Remaining') },
]);
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-xl bg-black/40 backdrop-blur-sm border border-purple-500/20 p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <h2 class="text-sm font-bold text-white">{{ $t('OBS Browser Source URL') }}</h2>
            </div>
            <p class="text-xs text-gray-500 mb-1">{{ $t('Add this as a Browser Source in OBS. Use "transparent" background for overlay.') }}</p>
            <p class="text-xs text-gray-400 mb-2 [&_span]:font-mono [&_span]:font-bold [&_span]:text-purple-300"
               v-html="$t('Recommended OBS size: <span>:width x :height</span> px', { width: form.bar_width + 32, height: form.bar_height + 80 })"></p>
        ...
        <div class="rounded-xl bg-black/40 backdrop-blur-sm border border-white/10 p-4">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-sm font-bold text-white">{{ $t('Progress Bar') }}</h2>
                <div class="flex items-center gap-2">
                    <button @click="resetDefaults" class="px-2 py-1 text-[10px] font-medium text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 rounded transition-all">{{ $t('Reset Defaults') }}</button>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <h3 class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{{ $t('Bar Dimensions') }}</h3>
                        <span class="text-[10px] text-purple-300 font-mono">{{ $t('OBS: :width x :height px', { width: form.bar_width + 32, height: form.bar_height + 80 }) }}</span>
                    </div>
                    ...
                    <h3 class="text-[10px] font-bold text-gray-500 uppercase tracking-wider pt-2">{{ $t('Colors') }}</h3>
                    <div class="grid grid-cols-2 gap-2">
                        <label class="text-[10px] text-gray-400">
                            {{ $t('Fill Color') }}
                ...
                <div class="space-y-3">
                    <h3 class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{{ $t('Animation') }}</h3>
                    ...
                    <h3 class="text-[10px] font-bold text-gray-500 uppercase tracking-wider pt-2">{{ $t('Page Background (OBS)') }}</h3>
                    <div class="flex gap-1">
                        <button v-for="bg in [{v:'transparent',l:$t('Transparent')},{v:'chroma',l:$t('Chroma Green')},{v:'custom',l:$t('Custom')}]" :key="bg.v" @click="form.bg_type = bg.v"
                    ...
                    <h3 class="text-[10px] font-bold text-gray-500 uppercase tracking-wider pt-2">{{ $t('Text Position') }}</h3>
                    <div class="flex gap-1">
                        <button v-for="pos in [{v:'above',l:$t('Above Bar')},{v:'inside',l:$t('Inside Bar')},{v:'below',l:$t('Below Bar')}]" :key="pos.v" @click="form.text_position = pos.v"
                            class="px-2 py-1.5 text-[10px] font-medium rounded transition-all flex-1"
            ...
            <div class="mt-4 pt-4 border-t border-white/5">
                <h3 class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">{{ $t('Text Elements') }}</h3>
                <div class="space-y-1">
