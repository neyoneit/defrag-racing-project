...
<template>
    <Head :title="$t('Bulk Upload Models')" />
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-2xl md:text-3xl font-black text-white mb-2">{{ $t('Bulk Upload Models') }}</h1>
                <p class="text-gray-400">{{ $t('Upload multiple PK3 files at once. Metadata will be extracted automatically.') }}</p>
            </div>
            ...
            <div v-if="bulkResults" class="mb-8 space-y-4">
                ...
                <div v-if="bulkResults.success && bulkResults.success.length > 0" class="bg-green-500/10 border border-green-500/30 rounded-xl p-6">
                    <h3 class="text-lg font-bold text-green-400 mb-4">✓ {{ $t('Successfully Uploaded (:count)', { count: bulkResults.success.length }) }}</h3>
                    <div class="space-y-2">
                ...
                <div v-if="bulkResults.failed && bulkResults.failed.length > 0" class="bg-red-500/10 border border-red-500/30 rounded-xl p-6">
                    <h3 class="text-lg font-bold text-red-400 mb-4">✗ {{ $t('Failed (:count)', { count: bulkResults.failed.length }) }}</h3>
                    <div class="space-y-2">
            ...
            <form :action="route('models.bulk-upload.store')" method="POST" enctype="multipart/form-data" @submit="handleSubmit" class="space-y-6">
                ...
                <div class="bg-gradient-to-br from-white/10 to-white/5 rounded-2xl border border-white/10 p-6">
                    <label class="block text-sm font-bold text-white mb-3">{{ $t('Category') }}</label>
                ...
                <div class="bg-gradient-to-br from-white/10 to-white/5 rounded-2xl border border-white/10 p-6">
                    <label class="block text-sm font-bold text-white mb-3">
                        {{ $t('PK3 Files') }}
                        <span class="text-gray-400 font-normal">{{ $t('(select multiple)') }}</span>
                    </label>
                    ...
                    <div class="border-2 border-dashed border-white/20 rounded-xl p-8 text-center hover:border-blue-500/50 transition-all cursor-pointer"
                        ...
                        <p class="text-white font-bold mb-2">{{ $t('Click to select PK3 files') }}</p>
                        <p class="text-sm text-gray-400">{{ $t('or drag and drop multiple files here') }}</p>
                        <p class="text-xs text-gray-500 mt-2">{{ $t('Accepts .pk3 or .zip files (max 50MB each)') }}</p>
                    </div>

                    <!-- Selected Files List -->
                    <div v-if="selectedFiles.length > 0" class="mt-4 space-y-2">
                        <div class="flex items-center justify-between text-sm text-gray-400 mb-2">
                            <span>{{ $tc(':count file selected|:count files selected', selectedFiles.length) }}</span>
                ...
                <div class="flex items-center gap-4">
                    <button
                        ...
                        <span v-if="uploading" class="flex items-center justify-center gap-2">
                            ...
                            {{ $tc('Uploading :count file...|Uploading :count files...', selectedFiles.length) }}
                        </span>
                        <span v-else>{{ $tc('Upload :count Model|Upload :count Models', selectedFiles.length) }}</span>
                    </button>
            ...
            <div class="mt-8 bg-blue-500/10 border border-blue-500/30 rounded-xl p-6">
                <h3 class="text-lg font-bold text-blue-400 mb-3">ℹ️ {{ $t('Bulk Upload Info') }}</h3>
                <ul class="space-y-2 text-sm text-gray-300">
                    <li>• <span v-html="$t('<strong>Automatic metadata extraction</strong> - Model name, author, skins, sounds will be detected automatically')"></span></li>
                    <li>• <span v-html="$t('<strong>Original filename used</strong> - The PK3 filename will be used as the model display name')"></span></li>
                    <li>• <span v-html="$t('<strong>Auto-approved</strong> - Admin bulk uploads are automatically approved and visible')"></span></li>
                    <li>• <span v-html="$t('<strong>Supports both types</strong> - Complete models (with MD3) and skin-only packs are detected automatically')"></span></li>
                    <li>• <span v-html="$t('<strong>Base model detection</strong> - System automatically determines which Q3 model each upload is based on')"></span></li>
                </ul>
            </div>
        </div>
    </div>
