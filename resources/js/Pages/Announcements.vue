    ...
    <div class="">
        ...
        <div class="relative bg-gradient-to-b from-black/25 via-black/10 to-transparent pt-6 pb-96 pointer-events-none">
            <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pointer-events-auto">
                ...
                <p class="text-gray-400 mb-8">{{ $t('Stay updated with the latest news from Defrag Racing') }}</p>
            </div>
        </div>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16" style="margin-top: -22rem;">
            ...
            <div class="space-y-6">
                <article
                    ...
                    :key="announcement.id"
                    class="bg-black/40 rounded-2xl p-6 lg:p-8 shadow-2xl border border-white/10 hover:border-white/20 transition-all group"
                >
                    <!-- Announcement Header -->
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-2 text-sm text-gray-500">
                                ...
                                {{ $t('Posted :when', { when: date(announcement.created_at) }) }}
                            </div>
                        </div>
                    </div>
            ...
            <!-- Empty State -->
            <div v-if="!announcements || announcements.length === 0" class="bg-black/40 rounded-2xl p-12 shadow-2xl border border-white/10 text-center">
                ...
                <h3 class="text-xl font-bold text-gray-400 mb-2">{{ $t('No announcements yet') }}</h3>
                <p class="text-gray-500">{{ $t('Check back later for updates!') }}</p>
            </div>
        </div>
    </div>
</template>
