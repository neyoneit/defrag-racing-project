...
<template>
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <!-- Total Records -->
            <div class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-blue-600/20 to-blue-800/20 border border-blue-500/30 p-3 hover:scale-105 transition-transform duration-300" :title="$t('Total number of records held by clan members across all maps and modes')">
                ...
                <div class="relative flex items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-medium text-blue-300 whitespace-nowrap">{{ $t('Total Records') }}</span>
                </div>
            </div>

            <!-- Top 1 Positions -->
            <div class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-yellow-600/20 to-orange-800/20 border border-yellow-500/30 p-3 hover:scale-105 transition-transform duration-300" :title="$t('Total number of #1 world record positions held by clan members')">
                ...
                <div class="relative flex items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-medium text-yellow-300 whitespace-nowrap">{{ $t('World Records') }}</span>
                </div>
            </div>

            <!-- Top 3 Positions -->
            <div class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-purple-600/20 to-pink-800/20 border border-purple-500/30 p-3 hover:scale-105 transition-transform duration-300" :title="$t('Total number of top 3 podium finishes (1st, 2nd, or 3rd place) held by clan members')">
                ...
                <div class="relative flex items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-medium text-purple-300 whitespace-nowrap">{{ $t('Podium Finishes') }}</span>
                </div>
            </div>

            <!-- Longest Streak -->
            <div class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-green-600/20 to-emerald-800/20 border border-green-500/30 p-3 hover:scale-105 transition-transform duration-300" :title="$t('Longest consecutive days with at least one record set by clan members')">
                ...
                <div class="relative flex items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-medium text-green-300 whitespace-nowrap">{{ $t('Day Streak') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-gray-900/85 to-gray-950/90 border border-white/10 rounded-2xl p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="bg-black/40 backdrop-blur-sm border border-white/10 rounded-lg p-4">
                    <h3 class="text-sm font-bold text-white uppercase tracking-wide mb-3">{{ $t('General') }}</h3>
                    <div class="space-y-2">
                        <div v-if="statistics.hall_of_fame.record_king" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Record King') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.speed_demon" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Speed Demon') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.sharpshooter" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Podium Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.hot_streak" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Streak Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.most_active" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Most Active') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.map_explorer" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Map Explorer') }}</span>
                ...
                <!-- Physics & Weapons -->
                <div class="bg-black/40 backdrop-blur-sm border border-white/10 rounded-lg p-4">
                    <h3 class="text-sm font-bold text-white uppercase tracking-wide mb-3">{{ $t('Physics & Weapons') }}</h3>
                    <div class="space-y-2">
                        <div v-if="statistics.hall_of_fame.vq3_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('VQ3 Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.cpm_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('CPM Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.rocket_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Rocket Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.plasma_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Plasma Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.grenade_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Grenade Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.bfg_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('BFG Master') }}</span>
                ...
                <!-- Map Features -->
                <div class="bg-black/40 backdrop-blur-sm border border-white/10 rounded-lg p-4">
                    <h3 class="text-sm font-bold text-white uppercase tracking-wide mb-3">{{ $t('Map Features') }}</h3>
                    <div class="space-y-2">
                        <div v-if="statistics.hall_of_fame.slick_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Slick Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.teleporter_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Teleporter Master') }}</span>
                        ...
                        <div v-if="statistics.hall_of_fame.jumppad_master" class="flex justify-between items-center">
                            <span class="text-xs text-gray-400">{{ $t('Jumppad Master') }}</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-gradient-to-br from-gray-900/85 to-gray-950/90 border border-white/10 rounded-2xl overflow-hidden">
                <!-- Tabs -->
                <div class="flex border-b border-white/10 overflow-x-auto">
                    ...
                    >
                        <div>{{ $t('Total Records') }}</div>
                        <div class="text-xs text-gray-300 font-normal mt-0.5">{{ $t('All records held') }}</div>
                    </button>
                    <button
                        ...
                        class="flex-shrink-0 px-4 border-b-2 font-semibold transition-colors text-sm text-left h-[72px] flex flex-col justify-center"
                    >
                        <div>{{ $t('World Records') }}</div>
                        <div class="text-xs text-gray-300 font-normal mt-0.5">{{ $t('#1 positions') }}</div>
                    </button>
                    <button
                    ...
                    >
                        <div>{{ $t('Top 3') }}</div>
                        <div class="text-xs text-gray-300 font-normal mt-0.5">{{ $t('Podium finishes') }}</div>
                    </button>
                    <button
                    ...
                    >
                        <div>{{ $t('Activity') }}</div>
                        <div class="text-xs text-gray-300 font-normal mt-0.5">{{ $t('Last 30 days') }}</div>
                    </button>
                    <button
                        ...
                        class="flex-shrink-0 px-4 border-b-2 font-semibold transition-colors text-sm text-left h-[72px] flex flex-col justify-center"
                    >
                        <div>{{ $t('Map Diversity') }}</div>
                        <div class="text-xs text-gray-300 font-normal mt-0.5">{{ $t('Unique maps') }}</div>
                    </button>
                </div>
                ...
                <div v-if="paginatedLeaderboard.totalPages > 1" class="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-white/10">
                    ...
                    <span class="text-white text-sm">{{ $t('Page :current of :total', { current: leaderboardPage, total: paginatedLeaderboard.totalPages }) }}</span>
                    <button
            ...
            <div class="bg-gradient-to-br from-gray-900/85 to-gray-950/90 border border-white/10 rounded-2xl overflow-hidden">
                ...
                <div class="border-b border-white/10 px-4 py-3 bg-gradient-to-r from-purple-600/10 to-purple-800/10">
                    <h3 class="text-lg font-black text-white flex items-center gap-2">
                        <img src="/images/powerups/invis.svg" class="w-5 h-5" :alt="$t('Untouched')" />
                        {{ $t('Untouched WRs') }}
                    </h3>
                    <p class="text-xs text-gray-400 mt-1">{{ $t('Longest-held world records') }}</p>
                </div>
                ...
                <div class="p-4">
                        ...
                        >
                            <Link :href="`/profile/${record.user_id}`" class="flex items-center gap-2 flex-1 group">
                                ...
                                <div class="flex items-center gap-1 text-purple-400 flex-shrink-0">
                                    ...
                                    <span class="text-xs text-gray-500">{{ $tc('day|days', record.days_held) }}</span>
                                </div>
                            </Link>
                        </div>
                        <div v-if="paginatedUntouchedWrs.data.length === 0" class="text-center py-8 text-gray-500">
                            {{ $t('No untouched world records') }}
                        </div>
                    </div>
                    ...
                    <div v-if="paginatedUntouchedWrs.totalPages > 1" class="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-white/10">
                        ...
                        <span class="text-white text-sm">{{ $t('Page :current of :total', { current: untouchedWrsPage, total: paginatedUntouchedWrs.totalPages }) }}</span>
                        <button
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <!-- Top 10 Density -->
            <div v-if="statistics.overview.top10_count !== undefined" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-indigo-600/20 to-indigo-800/20 border border-indigo-500/30 p-3 hover:scale-105 transition-transform duration-300" :title="$t('Total number of top 10 leaderboard positions (1st through 10th place) held by clan members')">
                ...
                <div class="relative flex items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-medium text-indigo-300 whitespace-nowrap">{{ $t('Top 10 Positions') }}</span>
                </div>
            </div>

            <!-- Average WR Age -->
            <div v-if="statistics.overview.avg_wr_age_days !== undefined" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-amber-600/20 to-amber-800/20 border border-amber-500/30 p-3 hover:scale-105 transition-transform duration-300" :title="$t('Average age (in days) of all world records held by clan members - how fresh or old the records are')">
                ...
                <div class="relative flex items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-medium text-amber-300 whitespace-nowrap">{{ $t('Avg WR Age (days)') }}</span>
                </div>
            </div>

            <!-- Completion Rate -->
            <div v-if="statistics.overview.completion_rate !== undefined" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-teal-600/20 to-teal-800/20 border border-teal-500/30 p-3 hover:scale-105 transition-transform duration-300" :title="$t('Percentage of all available maps that clan members have set records on - shows map diversity and coverage')">
                ...
                <div class="relative flex items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-medium text-teal-300 whitespace-nowrap">{{ $t('Map Coverage') }}</span>
                </div>
            </div>

            <!-- Physics Split -->
            <div v-if="statistics.overview.vq3_count !== undefined && statistics.overview.cpm_count !== undefined" class="group relative overflow-hidden rounded-xl bg-gradient-to-br from-pink-600/20 to-rose-800/20 border border-pink-500/30 p-3 hover:scale-105 transition-transform duration-300" :title="$t('Distribution of records between VQ3 and CPM physics modes - shows which physics style the clan prefers')">
                ...
                <div class="relative flex items-center justify-center gap-2">
                    ...
                    <span class="text-xs font-medium text-pink-300 whitespace-nowrap ml-2">{{ $t('Physics Preference') }}</span>
                </div>
            </div>
        </div>

        <!-- Podium Sweeps -->
        <div v-if="statistics.special_sections && statistics.special_sections.podium_sweeps && statistics.special_sections.podium_sweeps.length > 0" class="bg-gradient-to-br from-gray-900/85 to-gray-950/90 border border-white/10 rounded-2xl p-8">
            <h2 class="text-3xl font-black text-white mb-6 flex items-center gap-3">
                <img src="/images/powerups/quad.svg" class="w-8 h-8" :alt="$t('Podium Sweeps')" />
                {{ $t('Podium Sweeps') }}
            </h2>
            <div class="text-sm text-gray-400 mb-4">{{ $t('Maps where multiple clan members secured top 3 positions') }}</div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="sweep in statistics.special_sections.podium_sweeps" :key="`${sweep.mapname}-${sweep.physics}-${sweep.mode}`" class="bg-black/20 hover:bg-black/30 rounded-xl p-4 transition-all">
                    ...
                    <div class="mt-2 text-yellow-400 font-semibold">{{ $tc(':count member in top 3|:count members in top 3', sweep.clan_members_in_top3) }}</div>
                </div>
            </div>
        </div>

        <!-- Contested Positions -->
        <div v-if="statistics.special_sections && statistics.special_sections.contested_positions && statistics.special_sections.contested_positions.length > 0" class="bg-gradient-to-br from-gray-900/85 to-gray-950/90 border border-white/10 rounded-2xl p-8">
            <h2 class="text-3xl font-black text-white mb-6 flex items-center gap-3">
                <img src="/images/weapons/iconw_gauntlet.svg" class="w-8 h-8" :alt="$t('Contested')" />
                {{ $t('Contested Positions') }}
            </h2>
            <div class="text-sm text-gray-400 mb-4">{{ $t('Closest world records - barely ahead of 2nd place') }}</div>
            <div class="space-y-2">
                <div v-for="record in statistics.special_sections.contested_positions" :key="record.id" class="flex items-center justify-between bg-black/20 hover:bg-black/30 rounded-xl p-4 transition-all">
                    <div class="flex items-center gap-4 flex-1">
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-xs text-gray-500">{{ $t('ms margin') }}</div>
                        </div>
                    </div>
                </div>
            </div>
