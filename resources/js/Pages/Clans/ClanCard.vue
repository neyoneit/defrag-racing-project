<script setup>
    ...
    const getStatTitle = (stat) => {
        const titles = {
            'total_records': t('Total number of records held by clan members across all maps and modes'),
            'world_records': t('Total number of #1 world record positions held by clan members'),
            'podium_finishes': t('Total number of top 3 podium finishes (1st, 2nd, or 3rd place) held by clan members')
        };
        return titles[stat] || '';
    };
</script>
...
<template>
    ...
    >
        ...
        <div v-if="clan.background" class="absolute inset-0 opacity-30 group-hover:opacity-40 transition-opacity duration-500 overflow-hidden rounded-xl pointer-events-none">
            <img :src="`/storage/${clan.background}`" class="w-full h-full object-cover" :alt="$t('Clan Background')" />
        </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <Link :href="route('clans.show', clan.id)" class="flex items-center gap-4 min-w-0 flex-1 group/link">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="flex items-center gap-2 text-sm">
                            <span class="text-gray-500">{{ $t('Founder:') }}</span>
                            <Link
                    ...
                    <div v-if="clan.featured_stats.includes('total_records')" :class="[statSize, 'group relative overflow-hidden rounded-xl hover:scale-105 transition-transform duration-300 bg-gradient-to-br from-blue-600/20 to-blue-800/20 border border-blue-500/30']" :title="getStatTitle('total_records')">
                        <Link :href="route('clans.show', clan.id)" class="relative flex items-center gap-2" @click.stop>
                            ...
                            <div>
                                ...
                                <div class="text-[10px] font-medium text-blue-300">{{ $t('Records') }}</div>
                            </div>
                        </Link>
                    </div>
                    ...
                    <div v-if="showWR && clan.featured_stats.includes('world_records')" :class="[statSize, 'group relative overflow-hidden rounded-xl hover:scale-105 transition-transform duration-300 bg-gradient-to-br from-yellow-600/20 to-orange-800/20 border border-yellow-500/30']" :title="getStatTitle('world_records')">
                        <Link :href="route('clans.show', clan.id)" class="relative flex items-center gap-2" @click.stop>
                            ...
                            <div>
                                ...
                                <div class="text-[10px] font-medium text-yellow-300">{{ $t('WRs') }}</div>
                            </div>
                        </Link>
                    </div>
                    ...
                    <div v-if="showPodium && clan.featured_stats.includes('podium_finishes')" :class="[statSize, 'group relative overflow-hidden rounded-xl hover:scale-105 transition-transform duration-300 bg-gradient-to-br from-purple-600/20 to-pink-800/20 border border-purple-500/30']" :title="getStatTitle('podium_finishes')">
                        <Link :href="route('clans.show', clan.id)" class="relative flex items-center gap-2" @click.stop>
                            ...
                            <div>
                                ...
                                <div class="text-[10px] font-medium text-purple-300">{{ $t('Podium') }}</div>
                            </div>
                        </Link>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    >
                        ...
                        <span v-if="windowWidth >= 1280" class="text-xs font-medium">{{ $t('Invite') }}</span>
                    </button>
                    ...
                    >
                        ...
                        <span v-if="windowWidth >= 1280" class="text-xs font-medium">{{ $t('Kick') }}</span>
                    </button>
                    ...
                    >
                        ...
                        <span v-if="windowWidth >= 1280" class="text-xs font-medium">{{ $t('Edit') }}</span>
                    </button>
                    ...
                    >
                        ...
                        <span v-if="windowWidth >= 1280" class="text-xs font-medium">{{ $t('Details') }}</span>
                    </button>
                    ...
                    >
                        ...
                        <span v-if="windowWidth >= 1280" class="text-xs font-medium">{{ $t('Requests') }}</span>
                    ...
                    >
                        ...
                        <span v-if="windowWidth >= 1280" class="text-xs font-medium">{{ $t('Transfer') }}</span>
                    </button>
                    ...
                    >
                        ...
                        <span v-if="windowWidth >= 1280" class="text-xs font-medium">{{ $t('Leave') }}</span>
                    </button>
                    ...
                    <button
                        ...
                        @click="DismantleClan"
                        class="flex items-center justify-center gap-1 px-2 py-1.5 bg-gradient-to-r from-red-600/50 to-rose-600/50 hover:from-red-600/60 hover:to-rose-600/60 border border-red-500/50 hover:border-red-500/70 rounded-lg text-white transition-all duration-300"
                        :title="$t('Dismantle Clan')"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                        </svg>
                        <span v-if="windowWidth >= 1280" class="text-xs font-medium">{{ $t('Dismantle') }}</span>
                    </button>
                    </div>
                </div>
            </div>
