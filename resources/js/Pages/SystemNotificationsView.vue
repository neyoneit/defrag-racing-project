...
<template>
    <div class="">
        <Head :title="$t('System Notifications')" />
        ...
        <div class="relative bg-gradient-to-b from-black/25 via-black/10 to-transparent pt-6 pb-16">
            <div class="max-w-8xl mx-auto px-4 md:px-6 lg:px-8">
                <div class="flex justify-between items-start flex-wrap gap-4">
                    <div>
                        <h1 class="text-2xl md:text-3xl font-black text-gray-300/90 mb-2">{{ $t('System Notifications') }}</h1>
                        <div class="flex items-center gap-2 text-gray-400">
                            ...
                            <span class="text-sm font-semibold">{{ $tc(':count notification|:count notifications', notifications.total) }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="max-w-8xl mx-auto px-4 md:px-6 lg:px-8 -mt-8">
            <div class="bg-black/40 rounded-xl overflow-hidden shadow-2xl border border-white/5">
                ...
                <div v-if="notifications.data.length === 0" class="p-12 text-center">
                    ...
                    <p class="text-gray-500">{{ $t('You don\'t have any system notifications yet.') }}</p>
                </div>
            </div>
        </div>
