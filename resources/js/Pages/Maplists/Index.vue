...
const sortOptions = computed(() => [
    { value: 'likes', label: t('Most Liked'), icon: 'heart' },
    { value: 'favorites', label: t('Most Favorited'), icon: 'star' },
    { value: 'most_views', label: t('Most Views'), icon: 'eye' },
    { value: 'most_maps', label: t('Most Maps'), icon: 'map' },
    { value: 'least_maps', label: t('Least Maps'), icon: 'map-minus' },
    { value: 'newest', label: t('Newest'), icon: 'clock' },
    { value: 'oldest', label: t('Oldest'), icon: 'clock-old' },
]);

const viewOptions = computed(() => [
    { value: 'public', label: t('Public') },
    { value: 'mine', label: t('My Maplists') },
    { value: 'favorites', label: t('My Favourites') },
    { value: 'likes', label: t('My Likes') },
]);
...
const deleteDraftItem = async (id) => {
    if (!confirm(t('Are you sure you want to delete this draft?'))) return;
    ...
    try {
        ...
        alert(t('Failed to delete draft'));
    }
};

const createMaplist = async () => {
    if (!newMaplistName.value.trim()) {
        alert(t('Please enter a maplist name'));
        return;
    }
    ...
    try {
        ...
        if (error.response?.data?.errors) {
            ...
            alert(t('Failed to create maplist'));
        }
    } finally {
...
<template>
    <div class="">
            ...
            <div class="max-w-8xl mx-auto px-4 md:px-6 lg:px-8">
                ...
                <div v-if="currentView !== 'public'" class="flex items-center gap-2 text-sm text-gray-400 mb-6">
                    ...
                    <span class="text-white">
                        <template v-if="currentView === 'mine'">{{ $t('My Maplists') }}</template>
                        <template v-else-if="currentView === 'favorites'">{{ $t('My Favourites') }}</template>
                        <template v-else-if="currentView === 'likes'">{{ $t('My Likes') }}</template>
                    </span>
                </div>
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h1 class="text-2xl md:text-3xl font-black text-gray-300/90 mb-2">
                            ...
                            <template v-else-if="currentView === 'favorites'">{{ $t('My Favourites') }}</template>
                            <template v-else-if="currentView === 'likes'">{{ $t('My Likes') }}</template>
                            <template v-else>{{ $t('Maplists') }}</template>
                            <span v-if="maplists?.total != null" class="text-lg font-semibold text-gray-500 align-middle ml-2">{{ $tc(':count found|:count found', maplists.total) }}</span>
                        </h1>
                        <p class="text-gray-400">
                            <template v-if="currentView === 'mine'">{{ $t('Manage and organize your personal map collections.') }}</template>
                            <template v-else-if="currentView === 'favorites'">{{ $t('Your favorited maplists from the community.') }}</template>
                            <template v-else-if="currentView === 'likes'">{{ $t('Your liked maplists from the community.') }}</template>
                            <template v-else>{{ $t('Discover community-curated collections of maps. Like playlists for your favorite maps!') }}</template>
                        </p>
                    </div>
            ...
            <div class="lg:flex lg:gap-6">
                ...
                <div :class="showMobileFilters ? 'block mb-4' : 'hidden'" class="lg:block lg:w-56 lg:mb-0 lg:sticky lg:top-[120px] lg:self-start" style="height: fit-content; max-height: calc(100vh - 136px); overflow-y: auto; scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.06) transparent;">
                    <div class="bg-black/45 backdrop-blur-xl rounded-xl border border-white/[0.08] divide-y divide-white/5">
                        ...
                        <!-- Sort Options -->
                        <div class="px-3 py-2.5">
                            <div class="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{{ $t('Sort') }}</div>
                        ...
                        <div v-if="page.props.auth.user" class="px-3 py-2.5">
                            <div class="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{{ $t('View') }}</div>
                            <div class="space-y-0.5">
                                <button
                        ...
                        <div class="px-3 py-2.5 author-dropdown">
                            <div class="flex items-center justify-between mb-1.5">
                                <div class="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">{{ $t('Author') }}</div>
                            ...
                            >
                                ...
                                <span v-else>{{ $t('Select authors...') }}</span>
                            ...
                            <div v-if="authorDropdownOpen" class="mt-1.5 bg-gray-900/95 border border-white/10 rounded-lg shadow-xl max-h-48 overflow-hidden flex flex-col">
                                ...
                                <div class="overflow-y-auto flex-1">
                                    ...
                                    <div v-if="filteredAuthors.length === 0" class="px-3 py-3 text-center text-gray-500 text-xs">{{ $t('No results') }}</div>
                                </div>
                        ...
                        <div v-if="page.props.auth.user && currentView === 'public' && myPlayLater" class="px-3 py-2.5">
                            <div class="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">{{ $t('Quick Access') }}</div>
                            <div class="space-y-0.5">
                ...
                <main class="flex-1">
                    ...
                    <div v-if="hasActiveFilters" class="mb-6 flex flex-wrap gap-2">
                        <div v-if="searchQuery" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/20 border border-blue-500/30 rounded-lg text-sm text-blue-300">
                            <span class="text-xs text-blue-400">{{ $t('Search:') }}</span> {{ searchQuery }}
                        ...
                        <div v-for="authorId in selectedAuthors" :key="'au-'+authorId" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-500/20 border border-purple-500/30 rounded-lg text-sm text-purple-300">
                            <span class="text-xs text-purple-400">{{ $t('Author:') }}</span> {{ getAuthorName(authorId) }}
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            <div v-for="draft in drafts" :key="draft.id" class="bg-gray-800/90 border border-yellow-500/30 rounded-lg p-4 hover:bg-gray-700/90 transition-all">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <span class="px-2 py-0.5 bg-yellow-600/20 text-yellow-400 text-xs rounded-full">{{ $t('Draft') }}</span>
                                </div>
                                ...
                                <p class="text-xs text-gray-500 mt-2">{{ $t('Last edited: :date', { date: new Date(draft.updated_at).toLocaleString($page.props.locale) }) }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-else class="flex flex-col items-center justify-center py-20 text-gray-400">
                        ...
                        <p class="font-semibold text-lg mb-2">{{ $t('No maplists found') }}</p>
                        <p class="text-sm mb-4">
                            <template v-if="hasActiveFilters">{{ $t('Try adjusting your search or filters.') }}</template>
                            <template v-else>{{ $t('Be the first to create a maplist!') }}</template>
                        </p>
        ...
        <DialogModal :show="showCreateModal" @close="attemptCloseModal" max-width="4xl">
            <template #title>
                <div class="flex items-center space-x-2">
                    ...
                    <span>{{ $t('Create New Maplist') }}</span>
                    <span v-if="addedMapCount > 0" class="text-xs text-gray-400 font-normal ml-2">{{ $tc('(:count map)|(:count maps)', addedMapCount) }}</span>
                </div>
            </template>
                ...
                <div class="flex gap-6">
                    ...
                    <div class="flex-1 space-y-4 min-w-0">
                        <div>
                            ...
                            <input
                                ...
                                :placeholder="$t('e.g., My Favorite Maps')"
                            />
                        </div>

                        <div>
                            <label for="maplist_description" class="block text-xs font-medium text-gray-400 mb-1">{{ $t('Description (Optional)') }}</label>
                            <textarea
                                id="maplist_description"
                                v-model="newMaplistDescription"
                                rows="2"
                                class="w-full bg-black/30 border border-white/10 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-500"
                                :placeholder="$t('Describe your maplist...')"
                            ></textarea>
                        </div>

                        <div>
                            <label for="maplist_maps" class="block text-xs font-medium text-gray-400 mb-1">{{ $t('Maps (One per line)') }}</label>
                            <textarea
                                id="maplist_maps"
                                ...
                                :placeholder="$t('Type map names manually, one per line...')"
                            ></textarea>
                            <p class="mt-1 text-xs text-gray-500">{{ $t('Type map names one per line, or use the search panel to find and add maps.') }}</p>
                        </div>

                        <!-- Validation Errors -->
                        <div v-if="validationErrors.length > 0" class="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                            <h4 class="font-medium text-red-400 mb-2 text-sm">{{ $t('Cannot create maplist - Map issues found:') }}</h4>
                            <div class="space-y-2">
                                <div v-for="(error, index) in validationErrors" :key="index" class="text-sm">
                                    ...
                                    <div v-if="error.suggestions && error.suggestions.length > 0" class="ml-4 mt-1">
                                        <p class="text-yellow-400 text-xs">{{ $t('Did you mean:') }}</p>
                    ...
                    <div class="w-72 shrink-0 flex flex-col bg-white/[0.02] rounded-xl border border-white/10 overflow-hidden">
                        <div class="p-3 border-b border-white/10">
                            <label class="block text-xs font-medium text-gray-400 mb-1.5">{{ $t('Search Maps') }}</label>
                            <div class="relative">
                        ...
                        <div class="flex-1 overflow-y-auto max-h-[360px] p-1.5 space-y-1">
                            ...
                            <div v-else-if="!mapSearchQuery.trim()" class="flex flex-col items-center justify-center py-8 text-center px-4">
                                ...
                                <p class="text-xs text-gray-500">{{ $t('Search for maps to add them to your maplist') }}</p>
                            </div>
                            <div v-else-if="mapSearchResults.length === 0" class="flex flex-col items-center justify-center py-8">
                                <p class="text-xs text-gray-500">{{ $t('No maps found') }}</p>
                            </div>
                            <button
                            ...
                            >
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p v-if="map.author" class="text-xs text-gray-500 truncate">{{ $t('by :author', { author: map.author }) }}</p>
                                </div>
            ...
            <template #footer>
                <div class="flex items-center gap-3">
                    ...
                    <button
                        ...
                        {{ creating ? $t('Creating...') : $t('Create Maplist') }}
                    </button>
                </div>
            </template>
        </DialogModal>
        ...
        <ConfirmationModal :show="showCloseConfirm" @close="showCloseConfirm = false">
            <template #title>
                {{ $t('Exit Maplist Creation?') }}
            </template>

            <template #content>
                {{ $t('Are you sure you want to exit maplist creation? Your progress has been auto-saved as a draft and you can continue editing it later.') }}
            </template>
