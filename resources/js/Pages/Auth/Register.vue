        ...
        <div class="relative bg-gradient-to-b from-black/25 via-black/10 to-transparent pt-6 pb-96 pointer-events-none">
            <div class="max-w-lg mx-auto px-4 sm:px-6 lg:px-8 pointer-events-auto">
                <div class="text-center">
                    <h1 class="text-2xl md:text-3xl font-black text-white mb-2">{{ $t('Create Account') }}</h1>
                    <p class="text-gray-400">{{ $t('Join the defrag racing community') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="relative z-10 flex justify-center px-4 sm:px-6 lg:px-8" style="margin-top: -22rem;">
            ...
            <div class="bg-black/40 backdrop-blur-sm rounded-2xl p-8 shadow-2xl border border-white/10">
                <form @submit.prevent="submit" class="space-y-5">
                    <!-- Username Field -->
                    <div>
                        <label for="username" class="block text-sm font-bold text-gray-300 mb-2">{{ $t('Username') }}</label>
                        <input
                            id="username"
                    ...
                    <div>
                        <label for="name" class="block text-sm font-bold text-gray-300 mb-2">
                            {{ $t('Display Name:') }} <span v-if="form.name" v-html="q3tohtml(form.name)"></span>
                        </label>
                        <input
                            id="name"
                            v-model="form.name"
                        ...
                        />
                        <p class="mt-1 text-xs text-gray-500">{{ $t('You can use Quake3 color codes: ^1Red^2Green') }}</p>
                    ...
                    <!-- Country Field -->
                    <div>
                        <label for="country" class="block text-sm font-bold text-gray-300 mb-2">{{ $t('Country') }}</label>
                        <CountrySelect
                            id="country"
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-sm font-bold text-gray-300 mb-2">{{ $t('Confirm Password') }}</label>
                        <input
                    ...
                    <div v-if="$page.props.jetstream.hasTermsAndPrivacyPolicyFeature" class="pt-2">
                        <label class="flex items-start cursor-pointer group">
                            ...
                            />
                            <span class="ml-3 text-sm text-gray-400 group-hover:text-gray-300 transition [&_a]:text-blue-400 [&_a:hover]:text-blue-300 [&_a]:underline"
                                  v-html="$t('I agree to the <a href=:url target=_blank>Terms of Service and Privacy Policy</a>', { url: route('terms.show') })"></span>
                        </label>
                    ...
                    >
                        <span v-if="!form.processing">{{ $t('Create Account') }}</span>
                ...
                <div class="relative my-6">
                    ...
                    <div class="relative flex justify-center text-sm">
                        <span class="px-4 bg-black/40 text-gray-400">{{ $t('Already have an account?') }}</span>
                    </div>
                </div>
