            ...
            <div class="space-y-3">
                ...
                <button
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <div class="font-medium text-white text-sm">{{ $t('Play Later') }}</div>
                            <div class="text-xs text-gray-400">{{ $t('Add to your Play Later list') }}</div>
                        </div>
                    </div>
                </button>
                ...
                <div v-if="maplists.length > 0" class="space-y-1.5">
                    <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider px-1">{{ $t('Your Maplists') }}</h3>
                        ...
                        <button
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <div class="text-xs text-gray-400">{{ $tc(':count map|:count maps', maplist.maps_count) }}</div>
                                </div>
                                <span v-if="maplist.is_public" class="text-xs text-green-400/80 bg-green-500/10 px-2 py-0.5 rounded-full">{{ $t('Public') }}</span>
                                <span v-else class="text-xs text-gray-500 bg-white/5 px-2 py-0.5 rounded-full">{{ $t('Private') }}</span>
                            </div>
                        </button>
                    </div>
                </div>
                    ...
                    <div v-if="showCreateForm" class="mt-3 space-y-3 bg-white/5 rounded-lg p-3 border border-white/5">
                        <div>
                            ...
                            <input
                                ...
                                :placeholder="$t('e.g., Favorite Strafe Maps')"
                                @keyup.enter="createAndAddToMaplist">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-400 mb-1">{{ $t('Description (Optional)') }}</label>
                            <textarea
                                v-model="newMaplistForm.description"
                                class="w-full bg-black/30 border border-white/10 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-500"
                                rows="2"
                                :placeholder="$t('Describe your maplist...')"></textarea>
                        </div>
                        <div class="flex items-center space-x-2">
                            <input
                            ...
                            <label for="is-public" class="text-sm text-gray-300">{{ $t('Make this maplist public') }}</label>
                        </div>
...
const addToPlayLater = async () => {
    try {
        ...
        if (playLater) {
            ...
            successMessage.value = t('Added to Play Later!');
            emit('added');
            setTimeout(() => {
                closeModal();
            }, 1500);
        ...
        if (error.response?.data?.error) {
            ...
            errorMessage.value = t('Failed to add to Play Later');
        }
    } finally {
...
const addToMaplist = async (maplistId) => {
    try {
        ...
        successMessage.value = t('Map added to maplist!');
        emit('added');
        setTimeout(() => {
            closeModal();
        }, 1500);
        ...
        if (error.response?.data?.error) {
            ...
            errorMessage.value = t('Failed to add map to maplist');
        }
    } finally {
        addingToMaplist.value = false;
    }
...
const createAndAddToMaplist = async () => {
    ...
    try {
        ...
        successMessage.value = t('Maplist created and map added!');
        emit('added');
        ...
        if (error.response?.data?.error) {
            ...
            errorMessage.value = t('Failed to create maplist');
        }
    } finally {
        addingToMaplist.value = false;
    }
