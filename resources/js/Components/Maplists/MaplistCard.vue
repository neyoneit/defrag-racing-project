<template>
    <div class="backdrop-blur-xl bg-black/40 rounded-lg overflow-hidden hover:shadow-xl transition-all duration-300 border border-white/5 hover:border-blue-500">
        <Link :href="`/maplists/${maplist.id}`" class="block">
            ...
            <div class="grid grid-cols-2 grid-rows-2 gap-0 bg-gray-950 h-48">
                <template v-if="maplist.maps && maplist.maps.length > 0">
                    <div v-for="(map, index) in maplist.maps.slice(0, 4)" :key="index" class="relative bg-gray-950 overflow-hidden">
                        <img
                            v-if="map.thumbnail"
                    ...
                    <div v-for="i in (4 - Math.min(maplist.maps.length, 4))" :key="`empty-${i}`" class="bg-gray-900 flex items-center justify-center overflow-hidden">
                ...
                <template v-else>
                    <div v-for="i in 4" :key="i" class="bg-gray-900 flex items-center justify-center overflow-hidden">
                        <svg class="w-8 h-8 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
                        </svg>
                    </div>
            ...
            <div class="p-4 overflow-visible">
                ...
                <p v-if="maplist.description" class="text-sm text-gray-400 line-clamp-2 mb-3">
                    {{ maplist.description }}
                </p>
