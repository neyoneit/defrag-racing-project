<script setup>
    ...
    const notifications = ref(page.props.systemNotifications)
...
<template>
    <div class="relative">
        <div ref="triggerRef" @click="open = ! open">
            ...
            <button v-else class="relative rounded-lg p-2 hover:bg-white/10 transition-all group">
                <div v-if="ping" class="absolute top-1 right-1 w-2 h-2 rounded-full bg-red-500"></div>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-gray-400 group-hover:text-white transition-colors">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.34 15.84c-.688-.06-1.386-.09-2.09-.09H7.5a4.5 4.5 0 1 1 0-9h.75c.704 0 1.402-.03 2.09-.09m0 9.18c.253.962.584 1.892.985 2.783.247.55.06 1.21-.463 1.511l-.657.38c-.551.318-1.26.117-1.527-.461a20.845 20.845 0 0 1-1.44-4.282m3.102.069a18.03 18.03 0 0 1-.59-4.59c0-1.586.205-3.124.59-4.59m0 9.18a23.848 23.848 0 0 1 8.835 2.535M10.34 6.66a23.847 23.847 0 0 0 8.835-2.535m0 0A23.74 23.74 0 0 0 18.795 3m.38 1.125a23.91 23.91 0 0 1 1.014 5.395m-1.014 8.855c-.118.38-.245.754-.38 1.125m.38-1.125a23.91 23.91 0 0 0 1.014-5.395m0-3.46c.495.413.811 1.035.811 1.73 0 .695-.316 1.317-.811 1.73m0-3.46a24.347 24.347 0 0 1 0 3.46" />
                </svg>
            </button>
                ...
                <div class="rounded-xl bg-gray-950 backdrop-blur-xl border border-white/10 p-4">
                    ...
                    <div style="max-height: 450px;" class="defrag-scrollbar">
                        <div v-for="(notification, index) in notifications" :key="index">
                            <div v-if="notification" class="flex gap-3 text-gray-300 p-3 hover:bg-white/5 rounded-lg transition-all text-sm">
                                ...
                                <div class="flex-1">
                                    ...
                                    <Link class="inline text-blue-400 hover:text-blue-300 font-semibold" :href="notification.url" v-html="q3tohtml(notification.headline || '')"></Link>
                                    <span> {{ notification.after }} </span>
                                </div>
                            </div>
                            <div v-if="notification && index < notifications.length - 1" class="border-t border-white/5 my-2"></div>
                        </div>
                        ...
                        <div v-if="notifications.length > 0" class="border-t border-white/10 mt-3 pt-3">
                            <Link :href="route('notifications.system.index')" class="block text-center text-sm font-medium text-blue-400 hover:text-blue-300 transition-colors">
