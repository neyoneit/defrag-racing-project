        ...
        <div class="sm:ml-4 flex-grow">
            ...
            <div class="w-full mt-1 flex justify-between mb-1">
                ...
                <div class="flex items-center text-gray-400 text-sm">
                    {{ $t('By') }} <Link class="ml-1 hover:text-gray-300 overflow-hidden truncate" :title="map?.author" :href="route('maps.filters', {author: map?.author ?? 'unknown'})">{{ map?.author ?? $t('Unknown') }}</Link>
                </div>

                <div class="flex items-center text-gray-400 text-sm" :title="date.format('MMMM Do, YYYY')">
                    {{ $t('Added :date', { date: date.fromNow() }) }}
                </div>
            </div>
